...
class Display:
    ...
    def render(self, sudoku, selected_square=False, easy_toggle=True, medium_toggle=False, hard_toggle=False, extreme_toggle=False, unfair_toggle=False):
        ...
        self.render_difficulty('Extreme', 349, 110, extreme_toggle)
        self.render_difficulty('Unfair', 424, 110, unfair_toggle)
...
class Sudoku:
    def __init__(self, difficulty='Easy') -> None:
        ...
        line = self.generate_board(difficulty)
        self.board = line[0]
        self.solved_board = line[1]
        self.solvable_board = copy.deepcopy(self.board)
    
    def generate_board(self, difficulty):
        df = pd.read_csv(os.path.join(current_dir, 'boards.csv'))
        
        filtered_df = df[df['Difficulty'] == difficulty]
        
        random_row = filtered_df.sample(1)
        
        board_line = random_row['Board'].values[0]
        solution_line = random_row['Solution'].values[0]
        
        board = [list(map(lambda x: int(x) if x.isdigit() else 0, board_line[i:i+9])) for i in range(0, 81, 9)]
        solution = [list(map(lambda x: int(x) if x.isdigit() else 0, solution_line[i:i+9])) for i in range(0, 81, 9)]
...
def run_sudoku():
    ...
    easy_toggle, medium_toggle, hard_toggle, extreme_toggle, unfair_toggle = True, False, False, False, False
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if sudoku.game_over():
                    ...
                    if 124 <= mouse_pos[0] <= 124 + 70 and  110 <= mouse_pos[1] <= 130:
                        ...
                        difficulty = 'Medium'
                        easy_toggle, medium_toggle, hard_toggle, extreme_toggle, unfair_toggle = False, True, False, False, False
                        sudoku = Sudoku(difficulty)
                    elif 274 <= mouse_pos[0] <= 274 + 70 and  110 <= mouse_pos[1] <= 130:
                        difficulty = 'Hard'
                        easy_toggle, medium_toggle, hard_toggle, extreme_toggle, unfair_toggle = False, False, True, False, False
                        sudoku = Sudoku(difficulty)
                    elif 349 <= mouse_pos[0] <= 349 + 70 and  110 <= mouse_pos[1] <= 130:
                        difficulty = 'Extreme'
                        easy_toggle, medium_toggle, hard_toggle, extreme_toggle, unfair_toggle = False, False, False, True, False
                        sudoku = Sudoku(difficulty)
                    elif 424 <= mouse_pos[0] <= 424 + 70 and  110 <= mouse_pos[1] <= 130:
                        difficulty = 'Unfair'
                        easy_toggle, medium_toggle, hard_toggle, extreme_toggle, unfair_toggle = False, False, False, False, True
                        sudoku = Sudoku(difficulty)
        ...
        sudoku.display.render(sudoku, selected_square, easy_toggle, medium_toggle, hard_toggle, extreme_toggle, unfair_toggle)
